...
public class GlaceraIce
{
	...
	public string[] RequiredItems = { "Glacera Ice Token" };
	...
	public void ScriptMain(ScriptInterface bot)
	{
		...
		ConfigureBotOptions(LagKiller: false, SkipCutscenes: false);
		...
		while (!bot.ShouldExit())
		{
			...
			TempItemFarm("Dark Ice", 1, "northstar", "Enter", "Spawn", questId, "Frost Invader");
			SafeQuestComplete(questId);
		}
	End:
		bot.Log($"[{DateTime.Now:HH:mm:ss}] Script stopped successfully.");
		StopBot("Daily Glacera Ice Token bot completed.");
	}
